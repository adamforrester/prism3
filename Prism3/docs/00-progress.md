...
**+27 (1797 → 1824, measured on `5efc36d`)** — and the split is the interesting half: **+10** from the
L-02 block, **+17** from `t-nonmono` joining the shared corpus array, where it is now swept by every
pre-existing all-contracts and L-01 assertion rather than only the new ones. Closes #557.
...
**Verification.** All gates: `regen --check` (88 byte-match), `test.ts` 1824/0, `mcp-test.ts` 49/0,
...
**Two mutations that were NOT caught, and are unreachable rather than uncovered** (review's finding,
re-verified): `ratio >= min` → `ratio > min` in `guardFrom` needs an origin sitting exactly at its min, and
no origin in the corpus is within 0.005 of one; and `linkGuard` using `cfg.secondaryMin` instead of
`p.semanticMin` leaves all 3,600 roles byte-identical, because no current `iconContrast: '3:1'` profile
produces a link state where the two bars pick different steps. Neither is a defect, but the comment at the
`linkGuard` line states a per-profile guarantee that **nothing currently distinguishes** — if a future
profile makes those bars diverge, that is where it breaks silently. Named here rather than gated, since a
test would assert a distinction the engine cannot yet produce.

**Also worth keeping from the review:** the ramp has **exactly one** inversion, `450 → 500`. A single bad
step in twenty is why this survived every gate — nothing about the failure looks systemic when you read it.
