...
`CONTRACT_VERSION` stands at 2.0.0 — the case the version split exists for). `test.ts` 1758 →
**1769**, regen **88 in sync**, NB regression 11/11, all gates green.
...
Measured across the corpus: **4 of 5 brands failed in light mode** — `nb`, `aurora`, `wendys`,
`minimal`. Only harbor passed, because its warm off-white canvas holds the floor high. That set
includes `minimal` (the sparsest accepted input) and the web start screen's own default brand, so
**every new user was getting it.**
...
All new gates were mutation-tested: reverting `semanticInk` fires 10 failures across (10b), (10c) and
the Figma fixture rows. The mutation also showed the fix repairs **hc-light** brand ink (6.21–6.85 vs
its 7:1 bar) that no gate had been watching.
