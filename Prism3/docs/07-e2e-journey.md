...
| 4 — **MCP exposure / Figma pipeline** | the surface agents actually call | ✅ built — `engine/mcp.ts` (6 tools, CI-gated by `mcp-test.ts`) and the Figma plugin write path (`apps/plugin/src/write-figma.ts`) both exist |
...
3. **Plugin ownership.** ✅ RESOLVED (2026-07-01, see `08-theming-interfaces` §2): a **new**
   Prism3 plugin built on the engine core, *not* an evolution of the existing theming plugin —
   the core is reused (never re-implemented), the plugin is a fresh materialization + control
   shell. Also locked (`08` §3): **near-continuity** between the Figma plugin and the web
   playground via one shared **lever manifest** (`08` §4), not two hand-maintained UIs.
