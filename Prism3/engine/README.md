...
- `test.ts` — color-math invariants + extreme-brand contract smoke tests + typography/shadow/layout/gradient/surface-model + harshness + typography-weights/links + design.md-parser/CLI (aurora faithfulness byte-diff + harbor coverage) + standard-dialect/classifier/x-prism3 (Wendy's) invariants (202 checks).
...
- `regen.ts` — the single entry point that regenerates every committed generated artifact (all eight steps, in dependency order), plus `--check`, the drift gate that snapshots, regenerates, byte-compares, and restores. Covers **85** artifacts: `out/**`, the three emitted `schema/` contracts, and the two engine-root reports (`modes-report.md`, `nb-regression-report.md` — both were outside the comparison until the CI pass). See #281.
- generated outputs (committed so results are reviewable without running; all 85 are gated by `regen.ts --check`, which **CI now runs on every PR** — see `.github/workflows/ci.yml`): `nb-regression-report.md`, `modes-report.md`, `out/{nb,aurora,harbor,wendys}.tokens.json`, `out/{nb,aurora,harbor,wendys}.ai.json`, `out/wendys-fidelity-report.md`, `out/tokens.html`, `out/figma/<id>/*`, and the three emitted contracts in `../schema/`: `lever-manifest.json`, `preview-spec.json`, `example-brands.json`. **`regen.ts --check` is what keeps this list true** — it is the only gate that reads the committed files.
