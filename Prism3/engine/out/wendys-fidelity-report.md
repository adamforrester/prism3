...
- **Aliases**: 912/912 resolve · **mode contrast contracts**: 432/432 hold.
...
- Taxonomy divergence is intentional (t-shirt→numbered); the engine covers most observed px, with off-rhythm values absent by design. A documented translation, not an alignment change.
...
- **Format divergence:** observed = one-layer CSS; engine = two-layer composite → Figma Effect Style, reduced in dark (lift-primary). A translation the exporter owns, not a like-for-like port.
