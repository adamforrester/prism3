...
| [`tools/`](tools/) | **Measurement harnesses** — runnable and committed. Today: [`exporter-comparison/`](tools/exporter-comparison/README.md), which runs both DTCG exporters over the same brand and classifies every difference. The measurement (`compare.ts`) reports and always exits 0; the assertable subset of it (`gate.ts`) is a CI gate. Kept as two files, because a tool answers a question and a gate asserts an answer. |
