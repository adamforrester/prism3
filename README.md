...
| [`skills/`](skills/) | The **shipped skills** — `prism3-theme` (authoring a brand) and `prism3-consume` (building UI from generated tokens), for an agent working with Prism3; and `prism3-build-component` (authoring a new component definition), for an agent working on the engine. Gated by `lint-skills.ts`, because a skill describing a stale API is worse than no skill. |
| [`tools/`](tools/) | **Measurement harnesses** — runnable and committed. [`exporter-comparison/`](tools/exporter-comparison/README.md) runs both DTCG exporters over the same brand and classifies every difference: the measurement (`compare.ts`) reports and always exits 0, while the assertable subset of it (`gate.ts`) is a CI gate. Kept as two files, because a tool answers a question and a gate asserts an answer. [`nest-exposed-cost/`](tools/nest-exposed-cost/measure.ts) measures what a `nest-exposed` nested part would cost the parent that exposes it — a measurement only, since nobody has decided what the ceiling ought to be. |
