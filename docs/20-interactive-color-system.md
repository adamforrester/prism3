...
Measured before deciding: inverse is not dark mode (96 tokens match their dark-mode counterpart across all four brands, 40 differ, clustered on `interactive.*.fill.*`/`on-fill` — a primary button on a dark band takes a near-white fill, the same button in full dark mode takes a mid-tone brand fill; collapsing the two would silently change forty values). The alias layer is brand-independent (all four brands ship the identical 36 inverse token names, zero divergence), so the surface collection holds no colour values at all — pure indirection, authored once, shared by every brand.
