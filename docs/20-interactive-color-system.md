...
**One trap for whoever re-verifies this.** `focus-ring`'s `paintKeys` slot is spelled `border.inverse` and did **not** move. It is a coordinate in the def's own variant space (`color: 'default' | 'inverse'` → `{slot}.{color}`), not a token path; the token path it resolves to is `color.appearance.inverse.border.focus`, and *that* moved, along with its `UNALIASED_DEF_BINDINGS` entry. A sweep that treats the two as the same string will either miss the rename or break the def.
