...
### 8.2 Decided (2026-09-07, #1317): the rungs are named for intensity, not for the floor they clear

§8.1 named the three rungs `large`/`body`/`enhanced` — WCAG's own words for the floors 3/4.5/7 — so the *name* claimed each rung "clears X:1". **The owner reversed that: the rungs are now `subtle`/`medium`/`strong`, named for their magnitude (weakest wash to darkest).** The reasoning is that the floor is measured against the **worst pixel of an unknown image**, and a real photograph's combined contrast under the wash depends on the specific image — so a name that promises "clears 4.5:1" over-claims a guarantee the engine cannot keep per-photo. `medium` says the true thing (a mid-strength wash) and leaves the contrast judgment with the designer and their image.

**Values are unchanged, and that is the whole point of the change being cheap.** The three floors stay in `VEIL_RUNGS` as the **default-picker** — each rung still derives to the least alpha step clearing its floor — so the emitted alphas are byte-identical (`dark 50/60/70, light 40/50/60`); only the six token *names* and their `$description` prose move. `subtle`←`large`, `medium`←`body`, `strong`←`enhanced`, preserving weakest→darkest order.

It is a contract rename (`CONTRACT_VERSION` MAJOR 9.4.0 → 10.0.0, the class `token-contract.ts --check` demanded), recorded in `DEPRECATIONS` so a consumer or codemod can follow it; the Figma variable rename derives from those entries and needs no `MATERIALIZATION_RENAMES` rule, because a rung rename moves the *role* and so projects to a real variable rename (a materialization rule would double-claim it). `ENGINE_VERSION` bumps to 0.59.0 for the name/emission change. `modes.ts`'s veil design note carries the same reasoning beside the code.
...
### 9.9 Decided (2026-08-28, #1140): one top-level `inverse.` group — `inverse(X) = inverse.` + X, and `on-` takes a ground

§9.2 normalized three spellings of the inverse marker into one *segment*, but left it at three different **depths**: `background.inverse.primary`, `interactive.primary.inverse.fill.rest`, `text.on-inverse.primary`. That is one concept in three positions, and it is not a cosmetic complaint — every consumer of the taxonomy had to encode the position. `isInverseRole` matched three shapes. The coverage register's "does this role have an inverse counterpart?" check had three candidate derivations, so it could not tell a family whose counterpart is named *correctly* from one named under a shape that happens to also be admitted. And a component author reading the tree had no rule to apply, only a table to memorize.

**Rule 1 — `inverse.` is the sole inverse marker, at the front, always.** All 113 inverse roles in `color.appearance` move to a single top-level `inverse.` group: `background.inverse.primary` → `inverse.background.primary`, `interactive.primary.inverse.fill.rest` → `inverse.interactive.primary.fill.rest`. The inverse form of any role is mechanically derivable — `inverse(X) = inverse.` + X, with no exceptions and nothing to look up.

**Rule 2 — `on-` takes a GROUND, never a role and never a rank.** This is the half of the decision that has to be a rule rather than a rename, because it is what stops the marker drifting back. `on-inverse` is **gone**: the `inverse.` prefix already says "on the inverse surface", so the 34 text and icon tokens fold in as `inverse.text.*` / `inverse.icon.*`. What survives is `on-` naming an actual ground — the enclosing role's fill (`on-fill`) or a status colour (`text.on-brand`, `on-danger`, `on-success`, `on-warning`, `on-info`). `on-primary` and `on-disabled` are refused, because "primary" is a rank and "disabled" is a state; neither is a colour you can measure ink against. Both rules are stated as comments in `modes.ts` and Rule 2 is a **gate arm** — `lint-context-nodes.ts` arm C — so the next name authored is checked rather than remembered.

**Two structural fixes, both measured on aurora before being made.** `border/inverse/default` is **dropped**: it was byte-identical to `border/inverse/primary` in light *and* dark, i.e. two names for one value, and it is deprecated onto `inverse.border.primary` rather than deleted. And `border.tertiary` + `inverse.border.tertiary` are **added** — border was the only surface/ink category stopping at secondary. Worth stating because it inverts the intuition: for border, `tertiary` is the **strongest** rung, not the faintest. `primary` is the mode's `borderTarget`, `secondary` is ×2.2, `tertiary` ×2.2 again; in HC that asks 21.78:1 and `pickClosest` clamps to the darkest available step. The two fixes cancel in the count, which is why the inverse total is 113 both before and after: 112 relocate, one duplicate leaves, one new rung arrives.

**This is a rename, not a trim, and not a promotion.** All 113 inverse roles stay. They also stay **appearance-tier value roles** — they do not gain guaranteed pointer-tier short names, because #1135 is a deliberate per-name decision made when a real consumer needs one, never a wholesale grant. No trim follow-up exists; §9.8's bounded set is the model and it is unchanged.

**The contract cost was measured, not assumed: MAJOR.** A rename is a remove plus an add, so the question is only whether any renamed path is *guaranteed* — and **104 of the 113 are** (0 are `brandDependent`). `CONTRACT_VERSION` 7.1.0 → **8.0.0**, with 113 `DEPRECATIONS` entries at that version (112 relocations + the dedup). `guaranteed` moves 685 → **687**: three additions (`color.appearance.border.tertiary`, `color.appearance.inverse.border.tertiary`, and the pointer row `color.border.tertiary`, which the surface tier generates automatically from the new non-inverse role) less the one removal.

**The defect this PR found, and it was in the migration record rather than the emission.** `version.ts`'s load-bearing rule is that a deprecation's `path` is history and never moves while `replacedBy` follows the **live** name. So the moment #1140 renamed the live paths, #1013's 113 short-name deprecations and #1140's own 113 value-tier deprecations began projecting **the same 226 Figma renames** — two `DEPRECATIONS` entries of different vintage describing one operation. `planVariableRenames` groups by target and reads two live rows with one target as `ambiguous-source`, which *refuses the group*: **all 113 inverse renames would have been refused in a designer's file holding the old names.** That is precisely the binding-stranding #893 sequenced the mechanism to prevent, arriving by a path nobody had a gate on. Fixed in `deriveVariableRenames` by collapsing rows that are identical in `(collection, from, to)` — 532 rows → 306 — keeping the **greatest** `since`, because that field answers "when did this Figma name stop being written", and the later era is the truthful answer.

**What the collapse does not fix, measured: 111 of the 113 rows migrate and 2 refuse.** `border/inverse/default` → `inverse/border/primary` (the dedup above) and `border/inverse/primary` → `inverse/border/primary` (an ordinary relocation) name one target, which is *genuine* fan-in rather than the same-operation duplicate the collapse removes — so `planVariableRenames` refuses both, and rightly for only one of them: the dedup needs the designer to choose which variable survives, while the relocation is in no doubt and is blocked purely because an unrelated dedup shares its target. Its bindings stay on a variable the engine no longer writes. Filed as **#1142**; the fix belongs in `planVariableRenames`, not in the deprecation table.

**The second register, and why the materialization gate now reads it.** #1140 is the first contract-visible **role** rename since `lint-materialization-renames.ts` shipped (#1039), and it produced 339 unaccounted removals with nothing at all wrong. The tempting fix — a `MATERIALIZATION_RENAMES` rule — is the wrong one: it would put two independently-derived records in front of one Figma operation, and they would drift. A rename is already recorded, in `DEPRECATIONS`; the gate simply was not reading it. So `accountFor` takes the contract's claims as a second source, and a key claimed by *both* a rule and the contract lands in `multiplyClaimed` and fails — which is what keeps the two registers from being written for the same rename. **#1140 therefore gets no materialization rule at all**, and that absence is the correct state, not an omission.
...
**The pointer tier is deleted; the value tier takes the short name.** `color.appearance` is renamed to `color` and `color.surface` — a single-mode collection of 130 rows, every one a 1:1 alias into the tier above it, inverse excluded — is removed. Every consumer binds `color/*` directly: `color/background/primary` for the default surface, `color/inverse/background/primary` for the inverse one. The short names are preserved *by the rename*, so a consumer holding `color.background.primary` is unaffected; what shortens is the inverse half, `color.appearance.inverse.border.focus` → `color.inverse.border.focus`.

**What the pointer tier was buying, and why the rename buys it more cheaply.** #1082 justified the split on **appearance-independence**: a consumer should reference a name that does not encode which appearance it resolves in, and a thin single-mode tier aliasing into a moded one delivers that. It does — but so does a moded collection whose *names* carry no appearance segment, which is what `color.appearance` always was. The tier bought a stable short **name**, and a rename buys the same name with no second collection, no 1:1 alias layer to keep in sync, no `MIRRORED_COLLECTIONS` projection table, and one place for a designer to look. #1135's question — *should this inverse role get a short name?* — had 113 instances and a per-name answer; with one collection every role has exactly one name and there is nothing to grant. #1136's — *is `color.surface` the right name?* — is answered by there being no collection to name.

**The contract cost was measured, not assumed: MAJOR, and larger than #1140's.** `CONTRACT_VERSION` 8.0.0 → **9.0.0** — **225 guaranteed paths removed, 104 added, 0 demoted**. The asymmetry is the shape of the change: the removals are the whole `color.appearance.*` surface *plus* the pointer tier's `color.*` rows that no longer exist as their own paths, and the additions are the 104 guaranteed inverse paths arriving at their shortened spellings. `guaranteed` moves 687 → **566**, `brandDependent` 255, `DEPRECATIONS` 676. `ENGINE_VERSION` 0.29.0 → **0.30.0**. Committed artifacts drop **111 → 108**: three files, the pointer tier's emission.

**The migration is a collection rename plus a removal, and the removal is not a fan-in.** `Variable.variableCollectionId` is `readonly` (`@figma/plugin-typings/plugin-api.d.ts:11454`), so no rename can re-parent a variable and the pointer tier's rows cannot be folded into the value tier's collection. `COLLECTION_RENAMES` therefore holds one entry — `color.appearance` → `color` — and a designer's `color.surface` is **left in place, orphaned**: its variables keep their names and their bindings, resolving to values frozen at 0.29.0, until they repoint. That is refuse-or-report rather than corruption, and it is unreported by any executor (no plan walks that collection), so it is asserted by name in both suites and filed as **#1152**. The narrow **0.26.x** window — where the pointer tier was briefly *called* `color` — refuses with `target-occupied`, which under the executor's atomicity rule applies none of the map; the recovery is one manual rename. #1142's dedup+relocation fan-in still refuses 2 of 113 rows, unchanged by this.

**#1150 — the roles are written in reading order, and `inverse` is last.** The Figma variables panel groups by name and follows **creation order**, which is write order, which is DTCG key-insertion order — so one list, `COLOR_FAMILY_ORDER`, decides the order in both formats. It is now the order a reader would ask for rather than the alphabetical order that fell out: **background, foreground, text, icon, interactive, disabled, border, scrim, veil, field, inverse.** Surfaces before ink, ink before its interactive states, chrome after, and the 113-row inverse block **last** so it never sits between two families a designer is comparing. The write-order→panel-order link is verified **by mutation in the repo** (reorder the list, re-emit, confirm both the DTCG key order and the Figma `variables[]` order follow); that the *panel* renders that order is the owner's Figma check and is not claimed here.

### 9.11 Decided (2026-08-29, #1134): the inverse-context axis is `surface`, everywhere, and its paint is the projector's

The bounded inverse set (§9.8) needed a component-tier convention, and this settles it. **An inverse component declares a `surface: 'default' | 'inverse'` axis**, projected as Figma members `surface=default` / `surface=inverse`. That is the one name the distinction takes — `focus-ring` carried it as `color` until this issue renamed it, and `color` is now removed from `VARIANT_AXES`.

**The inverse paint is the projector's, not the def's.** On a `surface=inverse` coordinate the projector rewrites each resolved `color.*` ref to its `color.inverse.*` counterpart — §9.9's `inverse(X) = color.inverse. + X`, applied once in `anatomy-figma.ts`. So an inverse component binds only the DEFAULT-ground roles and authors no `color.inverse.*` key. **Why a rewrite rather than authored keys:** a stateful component cannot spell the inverse end in the key vocabulary — `button`'s `primary.filled.fill.inverse` collides with the `{state}` segment by arity and `inverse.disabled.fill` with `{intent}`, both refused by `paintKeyErrors`, and the cross-cutting disabled branch bypasses `paintKeys` entirely. `focus-ring` (stateless, single-slot) *could* author `border.inverse` and did through #1133; #1134 dropped that key so both components use the one mechanism. The transform is keyed on the `surface` axis value, which is what makes it safe under `lint-paint.ts`'s reachability probe (a def relying on it binds only default-ground roles, so no key is inverse-only).

**A host drives a nested component's `surface` by name.** Because the axis is `surface` on both, `button`'s `focusRing` part declares `nesting: { …, variant: { surface: 'default' }, follow: ['surface'] }`: a `surface=inverse` button member nests the `surface=inverse` ring, so a button on a dark band gets the ring tuned for it (its own 1.4.11 3:1 contract). `variant` is the fallback for a structure-only plan. This closes the ring-on-inverse gap (#1156). A nester without a `surface` axis of its own (icon-button, switch, checkbox, radio) nests the fixed `surface=default` ring — the rename touched their nest coordinate (`color` → `surface`) even though they gain no inverse variant, because a nester references the ring's axis by name.

**Enforceable, not remembered.** A def with a `surface` axis paints through the projector, so `validateComponentDef` never resolves the inverse leg; a `test.ts` arm asserts every colour role such a def binds has an `inverse.` counterpart in the emitted role set, with a committed mutation. `CONTRACT_VERSION` and `ENGINE_VERSION` are untouched — this is a component-tier + projector change over already-emitted roles, not a token-surface move.
