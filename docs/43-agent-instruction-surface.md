...
| 6 | US-English paragraph's four-trap restatement | ~1,900 of 2,410 | 1/2 | `packages/engine/lint-us-english.ts`'s own header, which the paragraph's own last sentence already says holds this: *"documented in the file's own header; read it before touching either scan."* | **Low.** The paragraph names its own redundancy. Keep the *rule* (US English, where, `NOT_EN_GB` is the fix for a false positive) and the two genuinely load-bearing open questions (the `apps/studio/src` comment carve-out, and whether it extends to `packages/engine/components/*.ts` — #849, explicitly undecided) resident; cut the trap-by-trap restatement. | ~1,900 |
