...
| Normalize `on-inverse` and `-inverse` to `.inverse.` *(superseded by §9.9 — one segment, but it left the marker at three depths)* | 2026-08-21 | `docs/20-interactive-color-system.md` §9.2 | #891 |
...
- **Levers that can delete guaranteed paths** are no longer an open question. #957's two, `outlineInteraction`
  ...
  #1639) and the body and caption defaults the form controls bind (#1681). One instance inside the sweep is a live
