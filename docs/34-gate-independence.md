...
4. **Check the named assertion, not the failure count.** "Among the failures" is not "the assertion
   it was written to fire." A non-zero count is not evidence on its own — a blank grep for the named
   assertion is a failed mutation, not a quiet pass, even while an unrelated arm fails and the count
   reads green (#978's P5, where the guard was unreachable by construction and the report came from a
   different arm entirely). Treat the named assertion's absence as the mutation not having run.
