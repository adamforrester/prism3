...
| **Paint is declared by the def, not built by the projector** | `paintKeys` on `ComponentDef` (#758, 2026-08-13). Seven grammars ship, not one — `icon` keys `tone.{tone}` with no slot segment, `field-message` keys `{tone}.{slot}`, the same axis name in the opposite order over a disjoint value set. `lint-paint.ts` gates the bindings; `component-schema.ts` gates the *order*, per binding, because `focus-ring` has no `size` axis and no census can reach it | **`paintOf` hardcoded `{intent}.{appearance}.{slot}`** — Button's axes as though universal, so **five of seven defs projected unpainted**, blocking Arc 2 steps 3 and 5 |
