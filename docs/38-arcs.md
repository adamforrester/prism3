...
| **Six of seven carry an `anatomy` block; four of those materialize** | `icon`, `focus-ring`, `button`, `icon-button`, `field-label` and `field-message` carry `anatomy` (`figmaAnatomyPlan` throws without one, `anatomy-figma.ts:235`); `text-field` is the one that does not. The four that materialize are the four with **both** `figmaProperties` and a `size` axis: `button` projects 648 plans, `icon-button` 162, `icon` 4, and `field-label` 4 as of #796 (`size` × `state`). `focus-ring` and `field-message` are the two that do NOT — both ship `figmaProperties` **absent**, and as of #796 for one stated reason rather than two: a def with one type scale is unprojectable at *any* coordinate, because `figmaAnatomyPlan` requires a declared `size` axis and `planComponentName` always writes `size=` (#795). For `focus-ring` a `variantAxes: ['color']` block would also validate cleanly and throw, but that is the second wall, not the first. **An `anatomy` block is necessary and not sufficient** — the distinction this row used to elide, and #796 is the pass that made it load-bearing rather than a caveat: both new field defs got their anatomy, and only one of them can be projected | **one, then two, then three, then four** — `32`'s *"Only one of five defs is materializable"* held until #734; this row's own headline said *"three… two of those"* while its evidence already cited `icon.ts` projecting, which is a def that cannot project without `anatomy` (corrected 2026-08-13 in #787, from a live census rather than a re-read). #796 raised it again by authoring two blocks and projecting one — and hit the same trap independently, which is the argument for counting from `componentDefs` rather than from this row |
...
| **Dependency A has its defs, and the gap moved rather than closed** | #741 authored both. `32`'s *"the schema cannot say so"* is answered — Button nests `focus-ring` via `kind: 'absolute'` + `nests` + `nesting`, and the ring's skin now binds engine tokens instead of a Figma file's placeholders. **What replaced it is more specific and further down:** `planComponentName` always writes `size=` while a ring has no size axis, so Button's `{color:'default'}` coordinate is satisfiable only by a **hand-built** component; and `PartDef` has no stroke field (#740). The paint wall is **closed** as of 2026-08-13: #758 moved the paint grammar out of `paintOf` and into the def as `paintKeys`, so the ring's stroke colour resolves — three *structural* walls remain, and `figmaProperties` stays absent | **no defs at all** — and the gap was believed to be in `anatomy.parts` |
...
2. ~~**`focus-ring`**~~ — **authored in #741, and NOT materializable, deliberately.** The def is real
   ...
   front of anything else that wants to nest a shared primitive by a non-size coordinate. **The first
   of the three — the `size=` coordinate — is filed as **#795** as of #796**, when `field-message`
   hit it too; its acceptance is that `focus-ring` and `field-message` both project. The other two
   (`planComponentName`'s unconditional `size=` is the same wall seen from the writer's side; `PartDef`
   has no stroke field, #740) are unchanged, so **#796 does not close this item.**
3. ~~**`field-label` and `field-message`** gain anatomy blocks.~~ — **authored in #796, and only
   `field-label` projects.** Both blocks are real and gate-checked; the step reads as half-done
   because the grammar and the projection are different things (§2's *necessary and not sufficient*).
   ...
   that could see it. `field-message` is authored and unprojected for `focus-ring`'s exact reason —
   one type scale, and `figmaAnatomyPlan` requires a `size` axis. **That wall is now filed as its own
   issue**, because it blocks two defs for one reason and has stopped being a per-def note.
