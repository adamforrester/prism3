...
2. ~~**`focus-ring`**~~ — **authored in #741; projects as of #795.** The def is real and its six
   ...
   offsets out of the Figma file. It now also ships `figmaProperties: { variantAxes: ['surface'] }` and
   projects **2 plans / 2 paint variables**, with Button's declared `{surface:'default'}` nest coordinate
