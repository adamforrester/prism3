...
| 6 | **Exemplars** (11) | **Behind** | *The report's term for a worked example an agent retrieves and copies from — few-shot rather than instructional, held at a stable address. Docs tell; exemplars show, and models copy more reliably than they follow.* No incorrect/correct pairs, no exemplar library. Cloudscape ships *"181 addressable few-shot exemplars at predictable URLs"*; daisyUI *"211 page architectures"* — **but re-verified 2026-08-11 and it is not what it sounds like:** those live in `daisyui-blueprint`, a **licensed** MCP, not the free library (~68 CSS-class components, 5-variant hero). The report describes each entry as *"purpose, sections, content order, actions, navigation, responsive behavior, interaction states, edge cases"* — a **page-level content model**, not a gallery. Cite it as evidence about artifact shape, not as a corpus. `preview.ts` (8 components / 25 variants) is a render spec, not an agent exemplar. |
