...
| 10 ✅ | #487 step 5 | **Done.** Write scope reaches the **component tier** — `apps/plugin/src/write-components.ts` (`applyComponentPlan`) materialises an `AnatomyPlan[]` into a live `COMPONENT_SET`. The first plugin lane driven by a plan the *paste* path also executes, so `test.ts` gates **axis parity** between the two (the axes Figma derived, the properties, the member coordinates, and the misses as sets — never the shared `planSetLayout`). Shaped like the *chunked* paste, not the single-shot one: find-or-create by name, skip by name, append, re-lay-out the union, resize — a plugin has no 45KB ceiling but a designer can press the button twice. **Not wired to a trigger yet** — materialising a set is a designer action of its own, not part of `apply-theme` (#483). |
