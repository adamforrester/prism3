...
> **As built** (the reworked surface & content model — see `06-surface-and-content-color-model.md`): the committed vocabulary is **`background`** (the thin page canvas: `primary/secondary/tertiary`, **tonal in both modes** — light is no longer all-white), **`foreground`** (the surfaces/fills layer — Prism2's `surface` renamed: `primary/secondary/tertiary` + bold semantic fills + `-subtle` tints; `foreground.primary` sits on `background.primary`), **`text`/`icon`** (ink: neutral + semantic + `-subtle` + `on-{semantic}` + `link`, with the cross-cutting `disabled.*` family carrying `disabled.text`/`disabled.icon`/`disabled.on-fill`), **`interactive.<color>.*`** (`primary`/`neutral`/`destructive` + opt-in `accent`, each with `fill` states, `on-fill`, `text`, `border`, `overlay.*`), **`border`** (`primary/secondary/tertiary/{semantic}/focus`), and — since #1140 — a single top-level **`inverse.*`** group holding the inverse form of all of the above (`inverse.background.*`, `inverse.text.*`, `inverse.interactive.<color>.*`, `inverse.border.*`, …). One marker in one position: `inverse(X) = inverse. + X`, with no `on-inverse` and no per-family `.inverse.` segment. The `surface`/`elevation` colour groups, `background.subtle`, `sunken`, and `quaternary` are **removed** — elevation is a foreground tier + a shadow, composed at the component layer. HC carries elevation by **border** (the neutral surface ladders flatten to the base). The earlier `foreground = neutral fill` meaning and the top-level **`action.*`** naming are superseded.
>
> **Structural encoding (2026-07-01 — see `06` §7b):** this whole role layer is emitted under **`color.*`** (the word "semantic" is not a path segment); the colour **primitives** live under **`palette.*`**. Each role is **one mode-agnostic token** — the `light` value is canonical in `$value`, and `dark`/`hc-light`/`hc-dark` are value overrides in `$extensions.prism3.modes` (each with its own contrast contract). So e.g. `color.background.primary` (not `semantic.light.background.primary`), aliasing `palette.*`. Maps 1:1 to a single Figma colour variable with Light/Dark/HC modes.
