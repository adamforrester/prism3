...
> **As built** (the reworked surface & content model — see `06-surface-and-content-color-model.md`): the committed vocabulary is **`background`** (the thin page canvas: `primary/secondary/tertiary` + an `inverse.*` ladder, **tonal in both modes** — light is no longer all-white), **`foreground`** (the surfaces/fills layer — Prism2's `surface` renamed: `primary/secondary/tertiary` + `inverse.*` + bold semantic fills + `-subtle` tints; `foreground.primary` sits on `background.primary`), **`text`/`icon`** (ink: neutral + semantic + `-subtle` + `on-{semantic}`/`on-inverse` + `link`, with the cross-cutting `disabled.*` family carrying `disabled.text`/`disabled.icon`/`disabled.on-fill`), **`interactive.<color>.*`** (`primary`/`neutral`/`destructive` + opt-in `accent`, each with `fill` states, `on-fill`, `text`, `border`, `overlay.*`), and **`border`** (`primary/secondary/{semantic}/focus` + an `inverse.{default,focus}` group). The `surface`/`elevation` colour groups, `background.subtle`, `sunken`, and `quaternary` are **removed** — elevation is a foreground tier + a shadow, composed at the component layer. HC carries elevation by **border** (the neutral surface ladders flatten to the base). The earlier `foreground = neutral fill` meaning and the top-level **`action.*`** naming are superseded.
>
