...
- [ ] `npx tsx packages/engine/lint-absolute-inset.ts` → clean — an absolutely-positioned part carrying an `inset` lands **outside** its parent (#801, the first instance of #802). Both causes #801 proposed were wrong: the lookup **resolves** (0 misses over the real 648-member set) and its miss path is reachable and **counted** (108 misses with the variable removed). What was missing is that nothing read the **number** — an offset of `0` is an error at no layer it passes: a valid FLOAT, binding nothing (Figma's `x`/`y` take no variable, so it travels as a name frozen to a literal at paste), writing without throwing, producing a structurally perfect component with 0 misses and a ring on the border it must be distinguishable from. EXPECTED walks the **def** (part → inset key → `tokens` → ref → variable name, restating the convention rather than calling `varOf`); ACTUAL is the plan's `absoluteInset` and the FLOAT it resolves to in each brand's **committed** export. It deliberately does not re-derive the executors' arithmetic — `test.ts` compares that against its own stub's inset and fails by name at 0, and its parity gate holds the plugin executor to the payload. Both directions, so it cannot pass over an empty set. A legitimately-zero offset (`ring.offset-field` is 0 by design) is admitted in `ZERO_OK` with a reason, authored for the same argument as the manifest. Stated limit: no browser and no Figma file, so a host that accepts a write and discards it is caught by the executors' read-backs, not here
