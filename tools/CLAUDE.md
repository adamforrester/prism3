...
`agent-link.ts` is the second mode (the agent link, §5 of the runbook): it prints the few-hundred-character
`use_figma` scripts that queue a command for the plugin RUNNING in the owner's Figma and read its result
back through the file's shared plugin data. Same arrangement as the paste loop, for the same reason — the
scripts are generated by `apps/plugin/agent-snippets.ts` and the gate that runs them against the plugin's
real `main.ts` is `apps/plugin/test-agent-link.ts`, an arm of the plugin's `test` step.

## `tools/figma-bridge/`

The agent link's transport B: `server.ts` is a stdio MCP server (`figma_status`, `figma_run`,
`figma_logs`) plus a hand-rolled RFC 6455 WebSocket server on `ws://localhost:17331`, which the plugin's
UI iframe connects to while the owner has the link on. It is a TOOL the owner registers with Claude Code,
not a measurement, and like `figma-mcp/` its gate lives in the plugin: `apps/plugin/test-agent-bridge.ts`
