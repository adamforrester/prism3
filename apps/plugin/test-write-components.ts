...
// per-member default from a set-wide one. (#1474: the defaults now NAME the slot per status — "Helper
// message" / "Error message" / "Warning message" / "Success message" — refining #1434's "This is a …
// message." wording, so the four members still read as one parallel set of placeholders.)
const fmCaption = (m: Node): string => String(fmKids(m).find((c) => c.type === 'TEXT')?.characters ?? '<none>');
const fmCaptions = fmMembers.map(fmCaption);
ok(fmCaptions.join(' | ') === 'Helper message | Error message | Warning message | Success message',
