...
// The COUNT is derived from the plans rather than pinned, because what #804 is about is "one set of its
// own name, every member added" — not the size of field-label's grid. It was 4 until #872 gave the def
// `weight` and `tone` axes and a third size rung (3 x 2 x 2 x 2 states = 24), and a hardcoded 4 would have
// failed on a change that says nothing about the property under test. `variants === added` is the part
// that carries the claim: every planned member reached the set, none silently dropped.
