 ...
 * live Figma.
 *
 *   npx tsx apps/plugin/test-write-styles.ts
 *
...
// `<root>/core/palette/primary/600`, not `palette/primary/600` (#1283 — the root is a lever). This is the one place a STYLE touches a
...
// #1283 moved aurora's root `prism` -> `ads` and this literal was RESTAMPED, not replaced by a read-back:
// the arm going red on a root move is the property, not a maintenance cost to design away.
ok(planAliases.every((a) => a.startsWith('ads/core/palette/')), `every carried alias is a rooted, core-tier palette name (e.g. ${planAliases[0]})`);
...
ok(bare.misses.every((m) => m.startsWith('ads/core/palette/')), 'each miss names the rooted palette variable it wanted — the name a designer would search for in the file, not a tail they would not find');
