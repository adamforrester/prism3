 ...
 * bumps MINOR (11.1.0 → 11.2.0): `type.label.lg.emphasis` is a guaranteed name ADD, and an add cannot
 * break an existing reference. 0.109.0 and not 0.106.0 because this merged onto post-#1471 main, which
 * took 0.108.0 — the next free ENGINE above it.
 *
 ...
 * reference — so a clean MINOR: 11.1.0 → 11.2.0 (layered on main's 11.1.0, the #1471 icon-role add).
 * Nothing is removed or retyped. `size.large.type` (a component binding, not a token path) re-points from
 * `type.label.md.emphasis` to the new rung; the `md` name is untouched and still bound elsewhere, so the
 * only guaranteed-surface move is the single ADD. `token-contract.ts --accept` records it and refuses
 * unless `CONTRACT_VERSION` was raised by exactly this MINOR first. (#1260)
 *
