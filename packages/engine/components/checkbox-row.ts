 ...
 * granularities — the bare box, the labeled row, the group — and the brief's headline job is deciding
 ...
 * `checkbox-row` (this def — the labeled row, renamed from `checkbox`), `checkbox-group` (the set, which
 ...
 *
 * **This def is the labeled ROW only** — the ~90% case. `Checkbox.Control` is now its OWN def
 ...
 * describes exactly one component of it — here the labeled Row, nesting the atom, filing the group.
 *
...
export const checkboxRow: ComponentDef = {
  ...
  props: [
    ...
    { name: 'value', type: 'string', required: false, description: 'The string submitted when checked (the platform default is `"on"`), and the element this row contributes to a Checkbox.Group\'s array. An unchecked box submits nothing at all rather than a false value — the missing-key trap.' },
  ...
  tokens: {
    ...
    // ── THE ROW'S TOP/BOTTOM PADDING (#1433b). Prism 2's checkbox-row root sits its content in
    // `padding {top: 12, bottom: 12, start: 0, end: 0}` (`reference/Prism2/component-specs/checkbox-row.json`),
    ...
    // centers inside a box this tall while the ROW stays top-aligned, so on a single line the control
    // reads centered and on a WRAPPING label it holds the first line instead of floating to the
    ...
    // instance is PINNED to the control square — 16/20/24 at comfortable density, 12/16/20 at compact — and does NOT
    // stretch to fill the taller `control-box` line box it is centered within. `control.size.*.height`
  ...
  // ── ANATOMY — THE LABELED ROW THAT NESTS THE CONTROL (#910, #1226 step 2) ──────────────────────
  //
  ...
  // centered on the first line rather than filling the taller box.
  //
  // THE ROW MUST NOT CENTER. On a wrapping consent label `align: center` floats the control to the
  ...
  anatomy: {
    ...
    parts: {
      ...
      row: {
        ...
        // center — on a wrapping consent label `align: center` floats the control to the middle of the
      ...
      // a line-tall box to center within, so the top-aligned row lands it on the FIRST line rather than
      ...
      label: {
        kind: 'text',
        type: 'size.{size}.text',
        // WRAPS rather than overflows (#1424). A consent line is often long and must wrap, not truncate
        // (`docs.dont` forbids centering it for that reason), so the label FILLS the row's main axis and
        // reflows while the top-aligned row holds the control on the first line (#1201). The row's `minWidth`
        // floor is what gives the fill something to resolve against — `anatomyErrors` requires it.
        wrap: true,
        note: 'The accessible name AND the second half of the hit target. Rich content in code (a consent line carries a link); a plain text node in Figma, where a nested link cannot exist. It WRAPS to a second line rather than overflowing (#1424) — it fills the row\'s main axis and reflows.',
      },
    },
    codeOnly: [
  ...
  docs: {
    usage: 'Use a single Checkbox.Row for one independent opt-in (a consent line, "Remember me"), and a Checkbox.Group to select any number — zero to many — from a bounded set. Use the indeterminate state for a select-all broadcasting partial selection over a table or tree. Compose FieldMessage below exactly as TextField does; the host wires the ids and the aria-describedby chain. Past roughly 7-10 options, scanning and vertical space suffer and a filtering multi-select combobox (not built yet) is the better control.',
    do: [
  ...
  ai: {
    ...
    commonPartners: ['checkbox-control', 'checkbox-group', 'field-label', 'field-message', 'focus-ring', 'button'],
  ...
  composition: {
    composesWith: ['checkbox-control', 'field-label', 'field-message', 'focus-ring'],
    alternativeTo: ['switch-row', 'radio-row', 'select'],
    replacesPatterns: [
  ...
  notes: {
    contested: [
      ...
      '`alignment` (top/baseline vs center) is in brief §15\'s variants block and is NOT an axis here. The brief calls top/baseline "the non-negotiable default, not center", and a non-negotiable default with one admissible value is not an axis — it has no dimension. It is a layout rule for the anatomy block to encode. The alternative is to declare it and accept an axis of one, which `modifiers` already demonstrates the cost of (#845). #1201 REFINED how that rule is encoded without weakening it: the control+label ROW is still top-aligned (never centered — `test.ts` #1009 half-1 enforces it), but the control now sits in a `controlBox` exactly one label line-box tall and centers WITHIN that box, so a single-line label reads centered and a wrapping one keeps the control on the first line. That is the construction #1009 filed and could not build, unblocked by baking the line-box to a fixed px per rung (`control.size.*.line-box`) — the one thing a Figma variable can hold that `fontSize × ratio` is not.',
