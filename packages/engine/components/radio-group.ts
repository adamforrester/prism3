...
export const radioGroup: ComponentDef = {
  ...
  status: 'draft',
  description:
    'A labeled set for choosing EXACTLY ONE from a small set of mutually exclusive, all-visible options. A required section label (FieldLabel) above a vertical stack of Radio rows. Owns the contract a single radio cannot — and without which the radios are not radios at all: the shared name that enforces exclusivity, the single scalar value, the single tab stop with roving focus, and all validation, announced once for the group. Use for 2 to about 7 options where seeing them all aids the decision; past that a filtering Select or Combobox scans better. Not any-number selection (CheckboxGroup), not a single independent opt-in (a lone Checkbox row), not an immediate-effect setting list (a list of Switches), not the same choice collapsed (Select) or in a compact skin (Segmented Control).',
  ...
  props: [
    ...
    { name: 'required', type: 'boolean', default: true, required: false, description: 'Whether an option must be chosen. Drives the nested FieldLabel\'s required marker (Prism 2\'s group defaults it on) and aria-required on the group. Group-level: an individual row never owns its own required. [HELD] the required-by-default follows Prism 2\'s spec; an OPTIONAL group must instead carry an explicit "None" option, because a radio cannot be deselected (`radio-row` emptyPattern) — a stray click is otherwise permanent.' },
    { name: 'size', type: "enum: 'small' | 'medium' | 'large'", values: ['small', 'medium', 'large'], default: 'medium', required: false, description: 'Scales the group — the label\'s type and every row (control circle, label ramp, gap) together, passed into the nested FieldLabel and rows by `follow`. [HELD] Prism 2\'s group is single-size; this def carries the family-universal size axis so the group is not frozen at one size.' },
    ...
    { name: 'disabled', type: 'boolean', default: false, required: false, description: 'Disables the whole set — every row and the label dim, driven by the group\'s context (the native disabled is the source of truth). [HELD] not projected as a Figma state; Prism 2\'s group shows no disabled treatment.' },
  ],
  ...
  content: {
    ...
    errorPattern: 'GROUP-LEVEL and specific — "Select a shipping method", stating what is wrong and how to fix it (SC 3.3.3), never "Invalid". The options stay neutral; the group carries the message. [HELD] the error VISUAL is not settled by Prism 2 and is not invented here.',
