...
const makeButton = (id: string, name: string, summary: string, description: string, family: IntentFamily): ComponentDef => ({
  ...
  anatomy: {
    ...
    parts: {
      container: {
        ...
        // `applyButtonLayout` before projection — this root goes FIXED at its floor and the label fills
        // the space between the icons with its text centered in that space. What is authored here is the
        // default, "Attached to label", unchanged. The per-size `minWidth` floor is written there too,
        // from the brand's heights, which is why this def authors none.
  ...
  docs: {
    ...
    do: [
      ...
      'Place the icons the way the brand\'s `buttonIcons` setting says: `attached` (Attached to label, the default) keeps them beside the label with the group centered; `edges` (Locked to edges) pins them to the button\'s edges and lets the label fill the space between, its text centered in that space, so a button with only a trailing icon has its label slightly left of center',
