 ...
 *   • DIMS — the whole geometric layer emitted as NINE FLOAT collections:
 *     `core-dimension` (fine-grid primitives), `space`/`radius`/`size`/`icon`/`control`/
 *     `border-width`/`focus` (all aliased into a `dimension/…` variable — the primitives are
 *     shared) + `opacity`
...
// `from './emit-figma'` importer are unchanged. The nine dims collections
// (`core-dimension`/`space`/`radius`/`size`/`icon`/`control`/`border-width`/`focus`/`opacity`) + the
