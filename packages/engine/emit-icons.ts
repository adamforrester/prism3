...
type Glyph = { name: string; source: string; path: string; fillRule?: string };
...
for (const [name, source] of Object.entries(ICON_SOURCES).sort(([a], [b]) => a.localeCompare(b))) {
  ...
  // The WINDING RULE, preserved when the source declares one (#1012). `fill-rule="evenodd"` is what
  // knocks an inner counter OUT of an enclosing shape — a disc with letters cut through it — which
  // nonzero winding fills back in solid. Only glyphs that need it carry it, so `absent` is the common
  // and correct case and is stored as nothing (nonzero is the SVG and Figma default). The `\bfill="`
  // probe above cannot match `fill-rule="`, so the two attributes do not collide.
  const fillRule = /\bfill-rule="([^"]*)"/.exec(paths[0])?.[1];
  if (fillRule !== undefined && fillRule !== 'evenodd' && fillRule !== 'nonzero') { shapeErrors.push(`${source}.svg: <path fill-rule="${fillRule}">, expected "evenodd" or "nonzero" — the only two SVG winding rules, and the only two a Figma import honours`); continue; }
  ...
  glyphs.push({ name, source, path: d, fillRule });
}
...
// SPARSE, and deliberately so: only a glyph whose source declared a NON-default winding rule appears.
// `nonzero` is the SVG and Figma default and every glyph absent from this map takes it, so the map is
// the exceptions and not a second copy of the vocabulary — a glyph gaining `evenodd` shows up here as
// one added line rather than as a field flipped across all 40 entries.
const fillRuleBody = glyphs
  .filter((g) => g.fillRule === 'evenodd')
  .map((g) => `  '${g.name}': 'evenodd',`)
  .join('\n');
...
/** Glyphs whose winding rule is not the \`nonzero\` default that SVG and Figma both assume. Sparse: a
 *  name absent here is \`nonzero\`; only a glyph that knocks an inner counter OUT of an enclosing shape
 *  (a lettered disc, say) needs \`evenodd\`, preserved from its source \`<path fill-rule>\` so it renders
 *  as drawn rather than filling solid (#1012). */
export const ICON_FILL_RULES: Partial<Record<IconName, 'evenodd'>> = {
${fillRuleBody}
};
