  ...
  for (const roleKey of orderedRoleKeys(Object.keys(lightMode.roles))) {
    ...
    for (const m of OVERRIDE_MODES) {
      ...
      modeOverrides[m] = rr.tint
        ? { $value: tintValue(rr), tint: tintRefs(rr), contrast: round(rr.ratio, 2), against: rr.against, ...washFields(rr), ...(rr.min > 0 ? { min: rr.min } : {}) }
        : { $value: `{${rr.path}}`, aliasOf: rr.path, contrast: round(rr.ratio, 2), against: rr.against, ...washFields(rr), ...(rr.min > 0 ? { min: rr.min } : {}) };
    }
    if (lr.tint) {
