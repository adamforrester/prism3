...
const UNREACHED_EXPLAINED: Record<string, string> = {
  ...
  'radio-control|focus-ring':
    'the fourth instance, and since #1348 it is on the ATOMIC control rather than the row — the radio split, and the ring moved with the painted circle into `radio-control`, which nests it inside its own `control` box (exactly as #1226 moved checkbox\'s ring into `checkbox-control` and #1354 moved switch\'s into `switch-control`). Still a nomination for the ring nested inside the control box, not a paint on any node radio owns. Four entries with one reason is now worth reading as a shape rather than four exemptions: every def that NESTS `focus-ring` will land here until `PartDef` gains a stroke field (#740).',
  'switch-control|focus-ring':
