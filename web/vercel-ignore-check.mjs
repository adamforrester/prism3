/**
 * Gate for `web/vercel-ignore.sh` (#474 follow-on).
 *
 ...
 * the day someone imports `ai-metadata.ts` into `web/src`, that file becomes a real bundle input
 ...
 *
 * Run: `node web/vercel-ignore-check.mjs`   (exits non-zero on drift; wired into CI)
 */
...
if (!block) {
  console.error('✗ could not find the EXCLUDED=( ... ) block in web/vercel-ignore.sh — did its shape change?');
  process.exit(1);
}
...
if (leaked.length) {
  ...
  console.error('\n  Remove them from EXCLUDED in web/vercel-ignore.sh.');
  process.exit(1);
}
